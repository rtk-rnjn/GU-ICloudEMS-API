...
from .__utils import ExponentialBackoff, compute_timedelta
...
class Loop(Generic[LF]):
    ...
    def _get_next_sleep_time(
        ...
        if self._sleep is not None and self._last_iteration is not None:
    ...
    def change_interval(
        ...
        *,
        ...
        hours: float | None = 0,
        time: Union[datetime.time, Sequence[datetime.time]] | None = None,
    ) -> None:
        if time is None:
            self._extracted_from_change_interval_11(seconds, minutes, hours)
        else:
    ...
    def _extracted_from_change_interval_11(self, seconds, minutes, hours):
        ...
        self._time: List[datetime.time] | None = None
